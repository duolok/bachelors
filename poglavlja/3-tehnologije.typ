...
безбедност и флексибилност. Ова имеплементација се базира на коришћењу модерног Раст екосистема безбедности меморије.
Овај одељак описује главне технологије коришћене у имплементацији: _Раст_ (енг. _Rust_) програмски језик као основу целокупног пројекта,
_Tokio_ асинхрони _runtime_ за неблокирајућу обраду података, _Ratatui_ библиотеку за креирање корисничког интерфејса у терминалу,
и _Crossterm_ библиотеку за вишеплатформску манипулацију терминала. Заједно, ове технологије чине робустан систем који
омогућава прикупљање и приказује системске метрике у реалном времену са минималним оптерећењем система.
...
безбедности типова (енг. _type safety_), безбедности меморије (енг. _memory safety_) и конкурентности.

Раст подржава више програмских стилова, а многе његове синтаксне и концептуалне особине као што су рад са
непроменљивим подацима, коришћење функција вишег реда, алгебарских типова и pattern matching-а, потичу из
функционалних језика. Истовремено, Раст омогућава и објектно-оријентисан начин програмирања преко структура,
енумерација, трејтова и метода.

Познат је по изузетно строгом приступу меморијској безбедности, све референце морају бити валидне.
...
Кључна особина која Раст разликује од других програмских језика је његов систем *власништва* (енг. _ownership_). 
...
животни век аутоматски закључује од стране компајлера (енг. _lifetimei elision_), понекад је потребно експлицитно дефинисати животни
век референци. Животни век се ознчава апострофом и генеричким параметром. Компајлер користи ове анотације да провери да ли су референце
...
*Висећи показивач* (енг. _dangling pointer_) је референца која показује на меморијску адресу која више није валидна, обично зато сто је објекат на који је показивала
...
повећавања привилегија (енг. privilege escalation) или до цурења информација уколико се показивач користи за писање у меморију. 

== `Tokio`

*Tokio* је асинхрони извршни оквир (енг. _asnyc runtime_) за Раст који омогућава писање неблокирајућих апликација. Пружа инфрастуктуру за 
извршавање асинхроних задатака (енг. _anyc tasks_).

=== Асинхрони модел извршавања 

Раст имплементира асинхроно програмирање кроз `async/await` синтаксу. Функције означене кључном речју `async` враћају објекат типа `Future` који 
...
`Tokio` користи планер (енг. _scheduler_)  заснован на алгоритму крађе посла (енг. _work-stealing scheduler_), који ефикасно распоређује 
асинхроне задатке на доступне нити. Сваки језгро (енг. _core_) има свој ред (енг. _queue_) задатака који извршава. Када је ред празан, језгро
проверава задатке од преосталих језгара и преузима њихов посао. Језгро ће отићи у стање спавања ако не може да пронађе задатак који би требао 
да се извршава.
...
користе за безбедну комуникацију између компоненати за прикупљање података и компоненти за приказ.

У овој имплементацији, `Tokio` се користи за неблокирајуће прикупљање метрика из различитих извора. Свака компонента за прикуопљање података 
ради као асинхрони задатак који периодично чита системске метрике. 

== `Ratatui`

*Ratatui* је Растова библиотека за креирање корисничког интерфејса за терминал (енг. _Terminal User Interface_).

=== Рендеровање 

Када разговарамо о развоју кориснчког интерфејса, разликујемо две главне парадигме: режим тренутног рендеровања (енг. _immediate mode rendering_) и 
...
`Ratatui` пружа богат скуп уграђених виџета који се могу комбиновати за изградњу сложених интерфејса:
...
*Crossterm* је Раст библиотека за манипулацију терминалом која омогућава писање вишеплатформских текстуалних 
...
boldiranja, италика, подвлачења, и других ефеката.
...
нивоа попут Crossterm-а за стварну интеракцију са терминалом. `Ratatui` подржава више _backend_-а (Crossterm, Termion, Termwiz), 
