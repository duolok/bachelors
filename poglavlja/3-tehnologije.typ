...
безбедности типова (енг. _type safety_), безбедности меморије (енг. _memory safety_) и конкурентности @rust-book @matsakis2014rust.
...
Познат је по изузетно строгом приступу меморијској безбедности, све референце морају бити валидне @jung2020rust.
...
свих објеката, што спречава уобичајене грешке у руковању меморијом и појаву трке до података (енг. _data race_) @rust-book.
...
преношења власништва @rust-book @rust-reference. Постоје два типа позајмљивања: непроменљиво позајмљивање (енг. _immutable borrow_) означено са `&T`, 
...
*Животни век* (енг. _lifetime_) је Раст-ова апстракција која описује колико дуго је референца валидна @rust-reference. Иако се у већини случајева
...
ослобођен из меморије @jung2020rust. Ово је једна од честих рањивости у језицима са ручним управљањем меморијом, као што су Ц и Ц++, која може довести до
...
бити експлицитно покренути позивањем методе `poll` или извршавањем кроз оквир као што је `Tokio` @tokio.
...
користе за безбедну комуникацију између компоненти за прикупљање података и компоненти за приказ @tokio.
...
`Ratatui` користи режим тренутног рендеровања са посредним баферима @ratatui. Ово значи да апликација за сваку слику мора експлицитно рендеровати све 
...
За целокупну листу виџета, потребно је погледати документацију @ratatui-widgets. 
...
емулатора терминала @crossterm. На UNIX системима, Crossterm користи ANSI escape секвенце за контролу терминала, док на 
