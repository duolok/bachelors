...
Сви колектори читају податке из Линукс `/proc` псеудо фајл 
система @proc-filesystem који пружа интерфејс ка `kernel` подацима. Сваки колектор имплементира `Collector` trait @rust-book и ради асинхроно 
користећи Tokio runtime @tokio.

=== Читање из /proc фајл система

Linux `/proc` фајл систем пружа текстуални интерфејс ка kernel подацима @proc-filesystem. Иако изгледају као обични фајлови, 
подаци се генеришу динамички приликом читања. Читање се обавља асинхроно користећи `tokio::fs` модул @tokio:
...
Овај приступ користи асинхрони I/O @rust-async-book што омогућава да друге операције настављају док се чека на читање фајла. 
Грешке се мапирају у специјализоване типове грешака који садрже контекст о томе који фајл је узроковао проблем.

=== CPU метрике

CPU метрике се прикупљају читањем `/proc/stat` фајла @proc-filesystem који садржи агрегиране статистике за све CPU-ове у систему, 
...
Информације о меморији се налазе у `/proc/meminfo` фајлу @proc-filesystem који садржи преко 40 различитих метрика. За 
ову имплементацију су релевантне следеће:
```
...
Прикупљање информација о процесима је најкомплекснија операција јер захтева читање више фајлова за сваки процес @proc-filesystem. 
...
фајлова. Асинхрони приступ @rust-async-book омогућава да се читања обављају паралелно, што значајно убрзава овај процес.


== Управљање процесима

Модул за управљање процесима омогућава слање различитих сигнала процесима @posix-signals и пружа заштиту од случајног гашења 
критичних системских процеса. За разлику од директног коришћења `libc::kill()` системског позива, имплементација 
користи `kill` команду преко Tokio `Command` API-ја @tokio, што пружа бољу изолацију и безбедност.
...
```,
caption: [Дефиниција типова сигнала за процесе]
)

=== Имплементација слања сигнала

Главна метода за слање сигнала користи `kill` команду преко Tokio Command API-ја @tokio. Овај приступ 
има неколико предности у односу на директно коришћење `libc::kill()`:

1. *Избегава несигуран код* — Не захтева unsafe блокове @rust-book. 
2. *Боља изолација* — Користи постојећу `kill` команду која је добро тестирана.
3. *Флексибилност* — Лакше подржава различите опције и сигнале @posix-signals.
...
Иако је могуће ручно имплементирати типове за грешке, `thiserror` @thiserror библиотека значајно поједностављује овај процес. 
Ова библиотека пружа процедурални макро који аутоматски генерише имплементације потребних trait-ова за custom error типове.

`thiserror` @thiserror пружа следеће могућности:
...
*#[from]* атрибут — Аутоматски генерише `From` trait имплементацију @rust-book, што омогућава коришћење `?` оператора за 
...
Имплементација користи `tokio::join!` @tokio за асинхроно прикупљање свих метрика. Прикупљање 
метрика је приказано у наредном листингу:
...
Кориснички интерфејс је подељен у табове, где сваки таб представља појединачну метрику која се прикупља @ratatui. 
Сваки таб се посебно имплементира са својим виџетима @ratatui-widgets и опцијама. На крају, неопходно их је повезати
