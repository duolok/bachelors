...
промовише раздвајање бриге и омогућава да промене у једном слоју имају минималан утицај
на друге слојеве.

У наставку је дат сажет опис свих главних слојева:

*Слој домена* (`oxyd-domain`) — Дефинише основне типове података, _trait_-ове и апстракције које користе остали модули. 

*Слој прикупљача* (`oxyd-collectors`) — Садржи имплементације за прикупљање различитих типова метрика из оперативног 
система. Сваки прикупљач чита податке из `/proc` фајл система и трансформише их у доменске типове.

*Слој управљања процесима* (`oxyd-process-manager`) — Пружа функционалност за управљање системским процесима, 
укључујући слање сигнала, терминацију и суспендовање процеса.
...
*Слој корисничког интерфејса* (`oxyd-tui`) — Имплементира терминалски кориснички интерфејс користећи `Ratatui` и 
`Crossterm` библиотеке. Одговоран је за приказ података и обраду корисничких улаза.
...
- Главни типови дефинисани у овом модулу укључују:
  ...
  - *SystemMetrics* — Структура која садржи све прикупљене метрике система у једном тренутку (CPU, меморија, мреža, диск).
...
- Поред структура података, модул дефинише и trait-ове:
...
`Collector` trait и специјализован је за читање одређеног типа података из `/proc` фајл система.
...
су `stat`, `status`, `cmdline`, `io`.

Сви прикупљачи раде асинхроно користећи Tokio runtime, што омогућава паралелно прикупљање различитих метрика 
без блокирања.
...
Модул `oxyd-process-manager` имплементира функционалност за управљање процесима.
...
прикупљачу и складишти резултате.
...
Модул `oxyd-tui` имплементира терминалски кориснички интерфејс користећи Ratatui и Crossterm. Архитектура 
је заснована на Flux обрасцу, који користи једносмеран ток података и предвидљиво управљање стањем апликације.

==== Flux архитектура

Flux је образац креиран од стране Facebook-a за развој корисничких интерфејса. 
...
*View* — Рендерује кориснички интерфејс на основу тренутног стања. Користи Ratatui виџете за приказ података и 
...
1. *Прикупљање* — Engine периодично активира све регистроване колекторе. 
   Сваки колектор асинхроно чита свој извор података из `/proc` фајл система.
...
3. *Дистрибуција* — Engine емитује нове метрике преко broadcast канала на који су претплаћене компоненте 
   корисничког интерфејса.

4. *Приказ* — TUI компонента прима ажурирање, ажурира своје интерно стање, и рендерује нови приказ.
...
*Observer* — Broadcast канали имплементирају observer образац где TUI компоненте могу да се претплате на 
ажурирања метрика.
...
За синхронизацију се користе Tokio примитиви:
...
Раст-ов систем власништва и позајмљивања гарантује да не може доћи до data race-ова, што чини конкурентни код 
